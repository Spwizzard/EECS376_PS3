...
float g_tangent_angle_deviation; //the angle of the left wall according to the radius_left and radius_tan_test (adjusted to be in terms of deviation from robot's angle)
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    ...
    float height = g_radius_tan_test * sin(DANG_TANGENT_APPROX);
    //use law of sines to approximate the angle of the left wall
    g_tangent_angle_deviation = asin(g_radius_tan_test * sin(DANG_TANGENT_APPROX) / height) - M_PI/2.0;
    ROS_INFO("radius of left wall = %f", g_radius_left);
    ROS_INFO("radius of tan test = %f", g_radius_tan_test);
    ROS_INFO("height diff = %f", height);
    ROS_INFO("angle of left wall = %f", g_tangent_angle_deviation);
...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
        ...
        ros::spinOnce();
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_tangent_angle_deviation > 0.1)) {
            //if here, we arne't following parallel to the wall precisely enough
            twist_cmd.linear.x = 0; //halt
            twist_commander.publish(twist_cmd);
            ROS_WARN("spinning to parallel");
            ROS_INFO("rotate dtheta = %f", g_tangent_angle_deviation);
            twist_cmd.angular.z = -YAW_RATE;
            twist_commander.publish(twist_cmd);
            double dt = fabs(g_tangent_angle_deviation / YAW_RATE);
        ...
        twist_commander.publish(twist_cmd);
        while ((g_clearance_tan_test < WALL_FOLLOW_RADIUS + 0.05) && (g_index_min_dist_ping >= g_index_tangent_left)) {
        ...
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left)) {
            ...
            while (g_clearance_tan_test > WALL_FOLLOW_RADIUS + 0.05) {
